...
| EJS_SFU_NETPLAY_ENABLED       | EmulatorJS bundle: `false` = OG EmulatorJS from cdn.emulatorjs.org (P2P netplay when `emulatorjs.netplay.enabled`); `true` = EmulatorJS-SFU from jsDelivr (SFU netplay, requires SFU server). Overrides config.yml `emulatorjs.netplay.ejs_sfu_enabled`. `SFU_NETPLAY_ENABLED` is a deprecated alias. |          | `false`                  |
